...
switch($request[3]){
  ...
  case 'add':
      if ($id = $IC->Research->QueueResearch($_SESSION['ruler']['id'], $_POST['research'])){
...
if ($queue = $IC->Research->LoadResearchQueue($_SESSION['ruler']['id'])){
  $data['queue'] = $queue;
}
$data['available'] = $IC->Research->LoadAvailableResearch($_SESSION['ruler']['id']);
