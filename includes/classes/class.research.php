...
class Research {
	...
	public function LoadResearchQueue($ruler_id){
		$q = "SELECT r.*, rq.id AS queue_id, rq.turns AS turns_left, rq.started, MD5(CONCAT(rq.id, '" . $ruler_id . "', rq.research_id,'".$this->config['salt']."')) AS hash FROM ruler_research_queue AS rq
	...
	public function QueueResearchRemove($ruler_id, $hash=false, $research_id=false){
		if ($hash){
			$q = "SELECT * FROM ruler_research_queue WHERE MD5(CONCAT(id, '" . $ruler_id . "', research_id,'".$this->config['salt']."')) = '" . $this->db->esc($hash) . "' LIMIT 1";
			if ($r = $this->db->Select($q)){
