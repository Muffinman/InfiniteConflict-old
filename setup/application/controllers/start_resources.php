...
class Start_resources extends CI_Controller{
  ...
  public function __construct() {
    ...
    $this->load->model('Start_resources_model');
  }

  public function index(){
    $this->smarty->assign('meta_title', 'Resource Editor');
    $this->smarty->assign('res', $this->Start_resources_model->read());

    $this->smarty->assign('content', $this->parser->parse('start_resources/index.tpl', array(), true));
  ...
  public function add(){
    $messages = array();
    $avail = $this->Start_resources_model->resources_avail();
    ...
    if ($this->formclass->hasErrors){
      ...
      if ($this->Start_resources_model->create($this->formclass->formData)){
    ...
    $this->smarty->assign('content', $this->parser->parse('start_resources/add.tpl', array(), true));
    $this->parser->parse('layout.tpl', array());
  ...
  public function edit($id){
    $messages = array();
    $resource = $this->Start_resources_model->read_single($id);
    ...
    if ($this->formclass->hasErrors){
      ...
      if ($this->Start_resources_model->update($id, $this->formclass->formData)){
    ...
    $this->smarty->assign('content', $this->parser->parse('start_resources/edit.tpl', array(), true));
    $this->parser->parse('layout.tpl', array());
  }
  ...
  public function delete($id){
    $messages = array();
    if ($resource = $this->Start_resources_model->delete($id)){
    ...
    $this->smarty->assign('content', $this->parser->parse('start_resources/delete.tpl', array(), true));
    $this->parser->parse('layout.tpl', array());
  }
