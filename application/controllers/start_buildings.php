...
class Start_buildings extends CI_Controller{
  ...
  public function __construct() {
    ...
    $this->load->model('Start_buildings_model');
  }

  public function index(){
    $this->smarty->assign('meta_title', 'Start Building Editor');
    $this->smarty->assign('buildings', $this->Start_buildings_model->read());

    $this->smarty->assign('content', $this->parser->parse('start_buildings/index.tpl', array(), true));
  ...
  public function add(){
    $messages = array();
    $avail = $this->Start_buildings_model->buildings_avail();
    ...
    if ($this->formclass->hasErrors){
      ...
      if ($this->Start_buildings_model->create($this->formclass->formData)){
    ...
    $this->smarty->assign('meta_title', 'Adding starting building ');

    $this->smarty->assign('content', $this->parser->parse('start_buildings/add.tpl', array(), true));
    $this->parser->parse('layout.tpl', array());
  ...
  public function edit($id){
    $messages = array();
    $building = $this->Start_buildings_model->read_single($id);
    ...
    if ($this->formclass->hasErrors){
      ...
      if ($this->Start_buildings_model->update($id, $this->formclass->formData)){
    ...
    $this->smarty->assign('content', $this->parser->parse('start_buildings/edit.tpl', array(), true));
    $this->parser->parse('layout.tpl', array());
  }
  ...
  public function delete($id){
    $messages = array();
    if ($building = $this->Start_buildings_model->delete($id)){
      $messages['success'][] = 'The building has been deleted.';
    }else{
      $messages['error'][] = 'There was an error deleting the building.';
    }
    $this->smarty->assign('messages', $messages);
    $this->smarty->assign('meta_title', 'Deleting start building');

    $this->smarty->assign('content', $this->parser->parse('start_buildings/delete.tpl', array(), true));
    $this->parser->parse('layout.tpl', array());
  }
